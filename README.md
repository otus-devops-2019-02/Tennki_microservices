...
# Kubernetes-2
- Подготовлены манифесты для запуска тестового приложения reddit
- Развернуто тестовое приложение в minikube
- Развернут k8s кластер в GKE вручную, развернуто тестовое приложение.
- Развернут региональный k8s кластер в GKE с помощью terraform. Конфигурационные файлы в каталоге kubernetes/terraform
    ...
    При создании включается addon kubernetes_dashboard

# Kubernetes-3
- Подготовлены манифесты для проверки различных вариантов сетевого взаимодействия компонентов k8s.
- Подготовлены манифесты для работы с PersistentVolume
- Создан манифест для создания Secret (kubernetes/reddit/ui-ingress-secret.yml) для ingress.
